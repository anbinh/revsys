...
module.exports = function(req, passport,pool,done) {
	pool.getConnection(function(err, connection) {
        ...
        if (req.query && req.query.q) {
            sql += " AND CONCAT_WS(' ',customers.first_name,customers.last_name) LIKE '%" + req.query.q + "%' ";
            search_user = true;
        } else {
            switch (user.role) {
                case "ADMIN" :
                    ...
                    break;
                case "STAFF" :
                    sql += " AND b.agent_id = " + pool.escape(user.id) + " ";
                    break;
                case "CUSTOMER" :
                    sql += " AND b.customer_id = " + pool.escape(user.id)+ " ";
                    break;
                default : 
        ...
        connection.query(sql, function(err, results, fields) {
            ...
            getTeamsAndAgents(user,connection,function(teams,agents){
                ...
                req.session.current_url = '/bookings';
    
    
                var siteConfig = {
                    title : "Next Bookings",
                    subtitle : "All available bookings in next 12 months",
                    bg : "/assets/img/background/bg1.jpg",
                    currPage : 'upcoming-bookings',
                    homeUrl : '/bookings'
                }
                // console.log("teams",teams);
    
                done({results: finalDat, teams : teams, agents : agents, siteConfig : siteConfig, user : user, query : req.query,search_user : search_user });
            });
